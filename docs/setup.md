...
New to Kafka? Start with [the three-file guide](start-here.md). Then follow [the full walkthrough](code-walkthrough.md) from the source API through Kafka, S3, and map playback.
...
and versioned metadata to separate Kafka topics. See [Stream format and migration](stream-format.md)
...
resolution (5–11), bike/dock totals and a 60-second refresh. Gray translucent cells show areas without reported stations in a
Toronto study rectangle (including water), not stations with zero bikes. Background
cells are generated for the visible area; dense requests show a limited nearby
grid while preserving the requested station resolution.
...
states. Flink processing is a possible future step. S3 archiving is configured separately in Confluent.
